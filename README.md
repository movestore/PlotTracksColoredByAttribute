...
`Attribute`: select the "Option 1: Color by 1 attribute" or "Option 2: Color by 2 attributes" 
 All available attributes of the study are displayed.
 
 "Option 1": select the attribute from the drop down list. this is mixed of continous and categorical attributes
...
`Panel`: the tracks can be either displayed on :
...
Check box `Add columns color hex and legend in the returned data`: the app adds extra columns to the returned move2 object.
